...
db = None
api_key_togetherAi = "##"
client = Together(api_key = api_key_togetherAi)
...
def query_llama_ApiTogetherAI(question, reponse):
    # Format the input prompt
    #formatted_prompt = f"Question: {question}\n\nRespons: {response}"
    formatted_prompt = f"""
                              Voici une question posée par un utilisateur et la réponse récupérée à partir de ChromaDB.
    ...
    print("Reponse genere par LLama depuis TogetherAI : \n", response.choices[0].text)
...
def query(query:str, role:str):
  ...
  if response == message_question_existante_autre_role:
    ...
    test_answer = query_llama_ApiTogetherAI(query, response)
...
if __name__ == "__main__":
    ...
    ## Tester LLama 2 With API Together AI
    question = "Quelle est la capitale de la france ?"
    reponse = "Paris"
    resp = query_llama_ApiTogetherAI(question, reponse)
    print('resp = ', resp)
